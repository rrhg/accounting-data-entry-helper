...
def get_transaction_from_ck_line( ck_line ):
    ...
    amount = try_convert_amount_to_float( amount, ck_line, line=ck_line, caller='utils.vendors.get_vendor_dict_from_ck_line()' ) # in case the pdf was read incorrectly & the amount includes unrecognized characters, ask to fix it here, instead of getting an error later 
    amount = str( amount )
