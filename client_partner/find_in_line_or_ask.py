...
def find_in_line_or_ask(line, previous_line, ptype=vendor, msg_if_not_found=''):
    partners_dict = get_all(ptype=ptype) # vendor will get all vendors & custmer will get all customers from json file
    partner_key = find_strings_in_line(line, previous_line, partners_dict)
    if partner_key:
        return partner_key
...
def find_vendor_in_line_or_ask(line, previous_line, msg_if_not_found=''):
    return find_in_line_or_ask(line, previous_line, ptype=vendor, msg_if_not_found=msg_if_not_found )
