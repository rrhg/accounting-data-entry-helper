...
import os
from utils.files import get_last_file_obj_in_dir
...
def get_txt_input_file_path(INTERNAL_BANK_STATEMENTS_DIR):
    ...
    filepath = INTERNAL_BANK_STATEMENTS_DIR / "bank_statement.txt"
    return filepath


def get_pdf_statement_file_path(INTERNAL_BANK_STATEMENTS_DIR):
    """  pdf bank statement to be used by pdf2txt.py(Tesseract) to be converted to text"""
    folder = 'path/to/folder'
    return get_last_file_obj_in_dir(folder)

    # another option
    # filepath = INTERNAL_BANK_STATEMENTS_DIR / 'file name goes here.pdf' # if you saves statement pdf to \accounting_clients\<name>\bank_statement\
